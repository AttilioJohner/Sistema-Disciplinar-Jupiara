-- =====================================================================
-- MIGRATION: Normalizar FKs - VERSÃO CORRIGIDA (SEM ALTERAR TIPOS)
-- Data: 2025-01-04
-- IMPORTANTE: Mantém tipo_medida exatamente como está
-- 
-- ESTRUTURA REAL:
...
-- alunos: codigo, "Nome completo", turma
-- =====================================================================
...
WITH medidas_norm AS (
  SELECT 
    ...
    m.codigo_matricula,
    m.tipo_medida, -- MANTÉM EXATO: "Fato Observado Negativo", etc
    m.data,
    m.especificacao,
    m.observacao
  FROM public.medidas m
...
SELECT 
  ...
  m.id AS medida_id,
  m.data,
  m.tipo_medida, -- ORIGINAL: "Fato Observado Negativo"
  m.especificacao,
  m.observacao,
  m.criado_em
FROM public.alunos a
...
RAISE NOTICE '✅ Views de medidas criadas (tipos originais mantidos)';
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  IMPORTANTE: Tipos de medida mantidos EXATAMENTE como estão!';
  RAISE NOTICE '   - "Fato Observado Negativo" permanece inalterado';
  RAISE NOTICE '   - Advertências, Suspensões mantidas como originais';
  RAISE NOTICE '';
...
DO $$
BEGIN
  RAISE NOTICE '🧪 TESTANDO VIEWS:';
  
  -- Mostrar alguns tipos de medida reais
  PERFORM 1 FROM public.v_medidas_por_tipo LIMIT 1;
  RAISE NOTICE '├── v_medidas_por_tipo: OK';
  
  PERFORM 1 FROM public.v_frequencia_acumulado_aluno_full LIMIT 1;
  RAISE NOTICE '├── v_frequencia_acumulado_aluno_full: OK';
  
  PERFORM 1 FROM public.v_resumo_disciplinar LIMIT 1;
  RAISE NOTICE '└── v_resumo_disciplinar: OK';
  
  RAISE NOTICE '';
  RAISE NOTICE '🔥 PRONTO PARA USAR!';
  
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'Erro no teste das views: %', SQLERRM;
END$$;
